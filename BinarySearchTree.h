...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  // NOTE: This function is used in the implementation of the ++ operator for
  //       the iterator code that is provided for you.
  // HINT: At each step, compare 'val' and the current node (using the
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
    ...
    if (less(val, node->datum)) {
      if (!node->left) { // min element
        return node; 
      }
      Node *left = min_greater_than_impl(node->left, val, less); // look for smaller element in left subtree
      if (!left) { // min element not in left subtree
        return node;  
      } 
      return left; // min element in left subtree 
    } 
    return min_greater_than_impl(node->right, val, less); // look for bigger element in right subtree      
  }
